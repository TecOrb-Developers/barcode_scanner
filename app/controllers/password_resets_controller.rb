 ...
 def forgot_password
     @user = User.find_by_email(params[:email])
      if @user
          ...
          render :json => { :Response_code => 500,:Response_message =>"This email id not registered"}
        end
    end
 ...
 def access_code
    ...
    if @user
      if@user.password_reset_token_sent_at < 2.hours.ago
       render :json => { :Response_code => 200,:Response_message => "Your access code has been expired"}	
  	  else
  	   render :json => { :Response_code => 500,:Response_message => "Your can reset your password now "}	
	   end
    else
