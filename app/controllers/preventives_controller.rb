	...
	def remove_preventive
		...
		if @preventive.present?
			...
			if @user.present?	
				...
				if (((@preventive.user_id == params[:user_id].to_i) and (params[:user_type]=="user")) or ((@preventive.member_id == params[:user_id].to_i) and (params[:user_type] == "member")))
					@preventive.destroy
				...
				render :json => {
	                            :response_code => 200,
	                            :response_message => @m,
	                            :user => @user.as_json(:only=>[:id,:name]).merge!(image: @user.image.url),
	                            :preventives => @user.preventives.as_json(:only =>[:id,:name])                     
	                          }
			else
			 	render :json => {
